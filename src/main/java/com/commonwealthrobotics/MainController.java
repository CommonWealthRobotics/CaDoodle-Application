...
public class MainController implements ICaDoodleStateUpdate, ICameraChangeListener {
	...
	void onExtrude(ActionEvent ae) {
		session.runExtrude(false);
		session.setKeyBindingFocus();
	}

	@FXML
	void onBend(ActionEvent ae) {
		session.runExtrude(true);
		session.setKeyBindingFocus();
	}
	...
	void onOrentAndFocusView(ActionEvent event) {
		session.submit(() -> {
			TransformNR scale = session.getFocusCenter();
			TransformNR orient = scale.times( new TransformNR(new RotationNR(0,
					0, -90)));
	...
	public void onChange(VirtualCameraMobileBase camera) {
		...
		session.onCameraChange(screenW, screenH, zoom, az, el, x, y, z, camera.getCamera().getFieldOfView());
	...
	private void setMaterialPanelPresent(boolean shouldBePresent) {
		boolean isPresent = shapeConfigurationBox.getPanes().contains(materialPanel);

		if (shouldBePresent && !isPresent) {
			shapeConfigurationBox.getPanes().add(materialPanel);
		} else if (!shouldBePresent && isPresent) {
			shapeConfigurationBox.getPanes().remove(materialPanel);
		}
	}
	...
	public void setAdvancedMode(boolean advanced) {
		...
		setMaterialPanelPresent(advanced);
		...
		pallet.updateShapesPalletOptions(advanced);
		BowlerStudio.runLater(() -> {
