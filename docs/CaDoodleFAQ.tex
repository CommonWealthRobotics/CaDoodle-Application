...
% Usage: \birdoverlay[width fraction][x offset][y offset][mirrored]
% width fraction: relative to \linewidth (default 0.15)
% x, y offsets: in cm, relative to page center (default 0cm)
% mirrored: 0 for normal, 1 for mirror left-right (default 0)
\newcommand{\birdoverlay}[4][0.15]{%
	\begin{tikzpicture}[remember picture, overlay]
		\pgfmathparse{#4 > 0 ? -1 : 1}%
		\edef\mirrorfactor{\pgfmathresult}%
		\node[inner sep=0pt] at ($(current page.center) + (#2, #3)$) {\scalebox{\mirrorfactor}{\includegraphics[width=#1\linewidth]{bird-logo.png}}};
	\end{tikzpicture}%
}
